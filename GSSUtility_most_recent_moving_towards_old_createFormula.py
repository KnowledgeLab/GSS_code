#
#     2017-07-13:
#        1. got rid of 'ddof=1' for IVs and only kept it for DV, to see if that would make a difference
#            - this made the effect go away
#        2. now i'm gonna try no ddof=1 only for DV and put it back in IVs
#            - the effect stays. so the only question is whether ddof=1 is used in the IVs
#        3. manually standrdizing variables to see if I can just do that and get rid of standardize() function altogether
#            - this makes the effect go away, ugh??7 whyh?
#      2017-07-14:
#        4. now gonna try to not standardize at all and see what happens: 
#                  hypothesis: effect returns
#                  but effect went away???
#    2017-08-13
#   1. ran without any standardization. (effect goes away)
#   2. now will center and stanrdize manually (effect goes away)
#   3. added "scale()" on variables -- effect goes away
#2017-08-14
#  4. now will try standardize(ddof=1)
#
#
#
#
...
# current changes:
# - took out 15 and replaced it with constant
# - took out ddof=1 everywhere <---- THIS TURNED OUT TO BE THE PROBLEM
...
def createFormula(dataCont, design, return_nominals=False, standardized=True):
    ...
    for col in design.columns[1:]: # don't include the DV in the RHS (the DV is the first element)!
        ...
        # all other cases (not in dict, in dict but C or CL), treat it as continuous
            ...
            formula += 'standardize(' + col + ', ddof=1) + ' 
...
def runModel(dataCont, year, DV, IVs, controls=[], custom_data=None, standardized=False):
    '''  
    inputs:
...
#     if design.dropna().shape[0] <= design.shape[1]:
    ...
    ################################################################################################################
    # NEW!!!!!!!!!
    # 2017-07-13: trying this because "standardize" in formulas in createFormula() seems to be messing up depending on "ddof" argument
    # standardize continuous variables
#    design[non_nominals] = (design[non_nominals] - design[non_nominals].mean()) / design[non_nominals].std()
    # now do DV
#    design[design.columns[0]] = (design[design.columns[0]] - design[design.columns[0]].mean()) / design[design.columns[0]].std()
    ####################################################################################################################################
...
#     keep only non-collinear columns
    ...
    # create formula
    formula = createFormula(dataCont, design, standardized=True)
    if not formula: 
