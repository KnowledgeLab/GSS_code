...
# now I want to limit the articles I run to a subset that meets the following conditions
# 1. It must've used only one year of data, i.e. len(article.GSSYearsUsed) == 1
articleClasses_subset = [a for a in articleClasses if len(a.GSSYearsUsed)==1]
# The condition above may be sufficient, but I'm also going to require that 
# 2. The one year used is the first year that at least one of the variables was available

'''
FIRSTYEAR_BY_VARIABLES = {}
for year, variables in VARS_BY_YEAR.iteritems():
    for name in variables:
        if name in FIRSTYEAR_BY_VARIABLES: continue
        else: FIRSTYEAR_BY_VARIABLES[name] = year

articleClasses_subset = [a for a in articleClasses_subset if 
                        np.array([FIRSTYEAR_BY_VARIABLES[name] == a.GSSYearsUsed 
                        for name in a.DVs + a.IVs + a.controls]).any()]
'''

print 'len of articleClasses_subset:', len(articleClasses_subset)

# begin estimating models   
#for article in random.sample(articleClasses_subset, 30):
for article in articleClasses_subset:
#for article in [a for a in articleClasses if a.articleID == 4454]:

    print 'Processing article:', article.articleID

    if np.array(article.GSSYearsPossible < article.GSSYearsUsed).any(): 
        print 'All unused years are earlier. Skipping.'
        continue 
